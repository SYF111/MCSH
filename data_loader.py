...
import csv
def read_csv(data1, balance=False):
    num_wrong_labels_sarcasm = 0
    num_wrong_labels_humor = 0
    num_wrong_labels_level = 0
    ...
    with open(data1, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            if row["texts"] == "texts":
                continue
            if row["sarcasm"] != "0" and row["sarcasm"] != "1":
                # row["sarcasm"] = "0"
                num_wrong_labels_sarcasm += 1
                # print('wrong_sarcasm')
                # print(row['sen_num'])
                continue
            if row["humor"] != "0" and row["humor"] != "1":
                # row["humor"] = "0"
                num_wrong_labels_humor += 1
                # print('wrong_humor')
                # print(row['sen_num'])
                continue
            if row["level"] != "0" and row["level"] != "1" and row["level"] != '3':
                # row["level"] = "0"
                num_wrong_labels_level += 1
                # print('wrong_level')
                # print(row['sen_num'])
                continue
            ...
            if row['sarcasm'] == '1':
                positive_sample.append([row["sentence"], row["sarcasm"], row['sen_num']])
            if row['sarcasm'] == '0':
                negative_sample.append([row["sentence"], row["sarcasm"], row['sen_num']])

            if row['level'] == '0':
                level_1.append([row["sentence"], row["level"]])
            if row['level'] == '1':
                level_2.append([row["sentence"], row["level"]])
            if row['level'] == '3':
                level_3.append([row["sentence"], row["level"]])

            if row["sen_num"].split("_")[0] == '10':
                valid_data.append([row["sentence"], row["sarcasm"], row['sen_num']])
            elif row["sen_num"].split("_")[0] == '9':
                test_data.append([row["sentence"], row["sarcasm"], row['sen_num']])
            else:
                train_data.append([row["sentence"], row["sarcasm"], row['sen_num']])
            ...
            if length <= 32:
                ...
                words_between_32_and_64 += 1
            else:
                words_over_64 += 1

            if len(row['sentence']) > longest_sentence_length:
                longest_sentence_length = len(row['sentence'])
        print(f"words_below_32:{words_below_32}:{words_below_32 / count:.2f}")
        print(f"words_between_32_and_64:{words_between_32_and_64}:{words_between_32_and_64 / count:.2f}")
        print(f"words_over_64:{words_over_64}:{words_over_64 / count:.2f}")
        print(f"longest_sentence_length:{longest_sentence_length}")
        print(f"average_sentence_length:{int(total_words / count)}")

        print(
            f"train_data_length:{len(train_data)} | valid_data_length:{len(valid_data)} | test_data_length:{len(test_data)}")
...
class DataLoader:
    ...
    # AUDIO_PICKLE = "./data/py_audio_300_audio_features.p"
    AUDIO_PICKLE = "./data/audio_feature_55.p"
    ...
    def __init__(self, config):
        ...
        if config.use_bert and config.use_target_text:
            # text_bert_embeddings = pickle_loader(self.BERT_TARGET_EMBEDDINGS)
            embeddings = pickle_loader(self.BERT_TARGET_EMBEDDINGS)
            text_bert_embeddings = {}
            for key , value in embeddings.items():
                bert_embedding_target = np.mean(value, axis=0)
                text_bert_embeddings[key] = bert_embedding_target
        else:
    ...
    def parseData(self, json, audio_features, video_features_file=None, context_video_features_file=None, text_bert_embeddings=None, context_bert_embeddings=None):
        ...
        for idx, ID in enumerate(json.keys()):
            # if self.config.use_bert and self.config.use_target_text and ID in text_bert_embeddings and not self.config.use_target_audio:
            #     self.get_data(ID, json, audio_features, video_features_file, text_bert_embeddings)
            # if self.config.use_target_audio and ID in audio_features and not self.config.use_target_text:
            #     self.get_data(ID, json, audio_features, video_features_file, text_bert_embeddings)
            # if self.config.use_bert and self.config.use_target_text and self.config.use_target_audio and ID in text_bert_embeddings and ID in audio_features:
            #     self.get_data(ID, json, audio_features, video_features_file, text_bert_embeddings)
            self.data_input.append((
                ID,
                np.reshape(audio_features[ID], (-1)) if audio_features else None,
            ...
            # self.data_output.append(random.randint(0,1))
